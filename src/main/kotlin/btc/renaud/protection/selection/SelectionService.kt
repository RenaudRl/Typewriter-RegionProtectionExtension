...
import btc.renaud.protection.selection.SelectionGeometry.verticalRange
...
class SelectionService(
    ...
    fun setPoint(player: Player, location: Location, index: Int) {
        ...
        if (session == null) {
            sendMessage(player, settingsRepository.snapshot(player).messages.selection.noSession)
            return
        }
...
private class SelectionSession(
    ...
    private val artifactKey: String = btc.renaud.protection.selection.artifactKey(artifact)
    ...
    fun matches(candidate: RegionArtifactEntry): Boolean = artifactKey == btc.renaud.protection.selection.artifactKey(candidate)

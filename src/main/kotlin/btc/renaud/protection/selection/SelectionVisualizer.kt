...
class SelectionVisualizer(
    ...
    private fun renderCuboid(shape: CuboidShape) {
        ...
        fun spawn(x: Double, y: Double, z: Double) {
            com.typewritermc.engine.paper.utils.particles.ParticleRenderer.render(
                player,
                org.bukkit.Location(player.world, x, y, z),
                particle.name,
                1,
                org.bukkit.util.Vector(0.0, 0.0, 0.0),
                0.0,
                dust
            )
        }
    ...
    private fun prepare(world: World?, color: Color): Particle.DustOptions? {
        ...
        val bukkitWorld = plugin.server.getWorld(worldName) ?: plugin.server.getWorld(player.world.name) ?: return null

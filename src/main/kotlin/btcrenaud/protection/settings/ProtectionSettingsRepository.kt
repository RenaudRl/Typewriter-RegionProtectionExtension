package btcrenaud.protection.settings

import com.typewritermc.core.entries.Query
import com.typewritermc.core.extension.annotations.Singleton
import com.typewritermc.engine.paper.entry.entries.get
import btcrenaud.protection.entry.settings.ProtectionMessageSettings
import btcrenaud.protection.entry.settings.ProtectionSettingsEntry
...
class ProtectionSettingsRepository {
    companion object {
        ...
        @Deprecated("Inject ProtectionSettingsRepository via Koin instead")
        fun snapshot(player: Player?): ProtectionSettingsSnapshot {
            return try {
                org.koin.java.KoinJavaComponent.get<ProtectionSettingsRepository>(ProtectionSettingsRepository::class.java).snapshot(player)
            } catch (e: Exception) {
                ProtectionSettingsSnapshot.default(
                    messages = ProtectionMessages(),
                    templates = ProtectionMessageTemplates(
                        MiniMessage.miniMessage(),
                        LoggerFactory.getLogger("ProtectionSettingsRepository"),
                        ProtectionMessageTemplateValues(
                            deniedAction = "<red>Denied Action{reason_line}</red>",
...
class ProtectionMessageTemplates(
    ...
    private fun render(key: String, template: String, placeholders: Map<String, Any?> = emptyMap()): Component {
        ...
        return runCatching { miniMessage.deserialize(template, resolver) }
            .getOrElse { error ->
                if (invalidTemplates.size < 1000 && invalidTemplates.putIfAbsent(key, template) == null) {
                    logger.warn("Failed to parse protection message '{}': {}", key, error.message)
                }
                Component.text(template)
            }
    }

    private fun indicatorComponent(enabled: Boolean, key: String, template: String): Component {
        if (!enabled) return Component.empty()
        return render(key, template)
    }
}

...
data class ProtectionMessages(
    ...
    data class CommandMessages(
        ...
        data class GeneralMessages(
            ...
            val flagNotFound: String = "<red>Flag '<flag>' not found.</red>",
            val flagSet: String = "<green>Flag <flag> set to <value> for region <region>.</green>",
            val flagCleared: String = "<green>Flag <flag> cleared for region <region>.</green>",
            val ownerAdded: String = "<green><player> added as owner of <region>.</green>",
            val ownerRemoved: String = "<green><player> removed as owner of <region>.</green>",
            val memberAdded: String = "<green><player> added as member of <region>.</green>",
            val memberRemoved: String = "<green><player> removed as member of <region>.</green>",
            val migrateFromWorldGuard: String = "<green>Migration complete: <migrated>/<found> regions migrated.</green>",
        )
...
object ProtectionMessageRenderer {
    ...
    fun render(template: String?, placeholders: Map<String, Any?> = emptyMap()): Component? {
        ...
        return runCatching { miniMessage.deserialize(raw, resolver) }
            .getOrElse { error ->
                if (loggedFailures.size < 1000 && loggedFailures.add(raw)) {

...
class BTCMobProtectionListener(
    ...
    private val btcMobsApi: Any? by lazy {
        try {
            val registration = Bukkit.getServicesManager().getRegistration(
                Class.forName("btcrenaud.btcmobs.api.BTCMobsApi")
            )
    ...
    private fun isBTCMobEntity(entity: org.bukkit.entity.Entity): Boolean {
        ...
        try {
            ...
            val registryMethod = api.javaClass.getMethod("getRegistry")
            val registry = registryMethod.invoke(api)
            val isActiveMobMethod = registry.javaClass.getMethod("isActiveMob", LivingEntity::class.java)
            return isActiveMobMethod.invoke(registry, entity) as? Boolean ?: false
        } catch (e: Exception) {

...
import btcrenaud.protection.listener.SchedulerCompat
import btcrenaud.protection.selection.SelectionGeometry.verticalRange
...
import btcrenaud.protection.selection.artifactKey
...
class SelectionService(
    ...
    fun startSession(player: Player, definition: RegionDefinitionEntry) {
        ...
        if (artifact is GlobalRegionArtifactEntry) {
            ...
            val shape = runBlocking { regionStorage.loadGlobal(artifact) }?.let { data ->
    ...
    private fun reuseOrCreateSession(
        ...
        existing?.close()
        val session = SelectionSession(plugin, player, regionStorage, artifact, definition, label, messages)
...
private class SelectionSession(
    ...
    private val artifactKey: String = btcrenaud.protection.selection.artifactKey(artifact)
    ...
    fun matches(candidate: RegionArtifactEntry): Boolean = artifactKey == btcrenaud.protection.selection.artifactKey(candidate)
    ...
    private fun preloadExistingShape() {
        val stored = runBlocking { regionStorage.load(artifact) }
        if (stored != null) {
            val mode = SelectionMode.entries.firstOrNull { it.name == stored.mode } ?: SelectionMode.CUBOID
            val shape = mode.computeShape(stored.nodes) ?: CuboidShape()
            applyShape(shape, stored.nodes, mode, announce = false)
            return
        }
        val current = KoinJavaComponent.get<RegionRepository>(RegionRepository::class.java).findById(artifactKey)?.shape
            ?: definition?.let { KoinJavaComponent.get<RegionRepository>(RegionRepository::class.java).findById(it.id)?.shape }
        if (current != null) {

...
import btcrenaud.protection.selection.content.ProtectionRegionContentMode
import btcrenaud.protection.selection.CuboidShape
import btcrenaud.protection.selection.CylinderShape
import btcrenaud.protection.selection.FlatPolygonShape
import btcrenaud.protection.selection.GlobalRegionShape
import btcrenaud.protection.selection.PolygonPrismShape
import btcrenaud.protection.selection.RegionShape
import btcrenaud.protection.selection.toTWPosition
import btcrenaud.protection.flags.RegionFlagRegistry
import btcrenaud.protection.flags.displayName
import btcrenaud.protection.flags.formatFlagValue
import btcrenaud.protection.flags.parseFlagValue
import btcrenaud.protection.flags.resolveFlagResolutions
import btcrenaud.protection.service.runtime.RegionInfoFormatter
...
import btcrenaud.protection.command.WorldGuardMigrationService
import btcrenaud.protection.settings.ProtectionMessageRenderer
import btcrenaud.protection.settings.ProtectionSettingsRepository
import btcrenaud.protection.settings.ProtectionMessages
...
private fun regionInfoFormatter(): RegionInfoFormatter = KoinJavaComponent.get(RegionInfoFormatter::class.java)
private fun settingsRepository(): ProtectionSettingsRepository = KoinJavaComponent.get(ProtectionSettingsRepository::class.java)
...
fun CommandTree.protectionCommand() = literal("protection") {
    ...
    literal("list") {
        executePlayer { player ->
            val listMessages = settingsRepository().snapshot(player).messages.commands.list
    ...
    literal("edit") {
        compatEntry<RegionDefinitionEntry>("manifest") { manifest ->
            executePlayer { player ->
                ...
                if (artifact == null) {
                    ProtectionMessageRenderer.render(
                        settingsRepository().snapshot(player).messages.selection.missingArtifact,
    ...
    literal("info") {
        compatEntry<RegionDefinitionEntry>("region") { regionRef ->
            executePlayer { player ->
                val messages = settingsRepository().snapshot(player).messages
        ...
        compatEntry<RegionDefinitionEntry>("region") { regionRef ->
            string("flag") { flagArg ->
                greedyString("value") { valueArg ->
                    executePlayer { player ->
                        val messages = settingsRepository().snapshot(player).messages.commands
                        val regionId = regionRef().id
                        val flagName = flagArg()
                        val rawValue = valueArg()

                        val flagKey = btcrenaud.protection.flags.RegionFlagKey.entries
                        ...
                        repository().setFlag(regionId, flagKey, rawValue)
                        ProtectionMessageRenderer.render(
        ...
        compatEntry<RegionDefinitionEntry>("region") { regionRef ->
            string("flag") { flagArg ->
                executePlayer { player ->
                    ...
                    val default = btcrenaud.protection.flags.parseFlagValue(flagKey, "")
                    repository().setFlag(regionId, flagKey, "")
                    ProtectionMessageRenderer.render(
        ...
        compatEntry<RegionDefinitionEntry>("region") { regionRef ->
            string("player") { playerArg ->
                executePlayer { player ->
                    ...
                    repository().addOwner(regionId, uuid)
                    ProtectionMessageRenderer.render(
        ...
        compatEntry<RegionDefinitionEntry>("region") { regionRef ->
            string("player") { playerArg ->
                executePlayer { player ->
                    ...
                    val uuid = target?.uniqueId?.toString() ?: targetName
                    repository().removeOwner(regionId, uuid)
                    ProtectionMessageRenderer.render(
        ...
        compatEntry<RegionDefinitionEntry>("region") { regionRef ->
            string("player") { playerArg ->
                executePlayer { player ->
                    ...
                    val uuid = target?.uniqueId?.toString() ?: targetName
                    repository().addMember(regionId, uuid)
                    ProtectionMessageRenderer.render(
        ...
        compatEntry<RegionDefinitionEntry>("region") { regionRef ->
            string("player") { playerArg ->
                executePlayer { player ->
                    ...
                    val uuid = target?.uniqueId?.toString() ?: targetName
                    repository().removeMember(regionId, uuid)
                    ProtectionMessageRenderer.render(
    ...
    literal("teleport") {
        compatEntry<RegionDefinitionEntry>("region") { regionRef ->
            executePlayer { player ->
                val commands = settingsRepository().snapshot(player).messages.commands
                val region = repository().findById(regionRef().id)
                if (region == null) {
    ...
    literal("inspect") {
        executePlayer { player ->
            val messages = settingsRepository().snapshot(player).messages.commands.inspect
    ...
    literal("migrate-from-worldguard") {
        withPermission("typewriter.protection.admin")
        executePlayer { player ->
            val migrationService = KoinJavaComponent.get<WorldGuardMigrationService>(WorldGuardMigrationService::class.java)

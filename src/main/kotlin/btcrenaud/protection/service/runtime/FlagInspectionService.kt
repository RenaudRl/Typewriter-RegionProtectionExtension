package btcrenaud.protection.service.runtime
...
import btcrenaud.protection.settings.ProtectionMessageTemplates
...
class FlagInspectionService {
    ...
    private inner class InspectionSession(
        ...
        private var task: btcrenaud.protection.listener.SchedulerCompat.TaskHandle? = null
        ...
        private fun render(snapshot: ProtectionSettingsSnapshot) {
            ...
            val cache = mutableMapOf<String, List<btcrenaud.protection.flags.FlagResolution>>()
        ...
        private fun notifyRegionEntries(messages: ProtectionMessages, regions: List<RegionModel>) {
            ...
            if (newRegions.isNotEmpty()) {
                newRegions.forEach { region ->
                    val formatter: RegionInfoFormatter = KoinJavaComponent.get(RegionInfoFormatter::class.java)
                    formatter.buildRegionInfoLines(region, messages).forEach { player.sendMessage(it) }
                }
            }
        ...
        private fun resolutionsFor(
            region: RegionModel,
            cache: MutableMap<String, List<btcrenaud.protection.flags.FlagResolution>>,
        ): List<btcrenaud.protection.flags.FlagResolution> = cache.getOrPut(region.regionId) {
        ...
        private fun buildBossText(
            ...
            region: RegionModel,
            resolutions: List<btcrenaud.protection.flags.FlagResolution>,
            index: Int,
            total: Int,
        ): Component {
        ...
        private fun scheduleNext() {
            ...
            task = btcrenaud.protection.listener.SchedulerCompat.runLater(plugin, player.location, delay) { tick() }
        }
